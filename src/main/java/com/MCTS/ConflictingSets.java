...
public class ConflictingSets {
    ...
    private static HashMap<ArrayList<Integer>,HashSet<Integer>> ConflictingSets;
    ...
    public static HashMap<ArrayList<Integer>,HashSet<Integer>> getAllConflicts() {
        return ConflictingSets;
    }

    private HashMap<ArrayList<Integer>,HashSet<Integer>> generateAllConflicts(){
        ...
        for(int i  = 0; i < allPossibleSets.size()-1; i++){
            ...
            for(int j = i+1; j < allPossibleSets.size(); j ++){
                if(hasCommonTile(allPossibleSets.get(i), allPossibleSets.get(j))){
                    ConflictingSets.get(allPossibleSets.get(i)).add(j);
                }
            }
        }
        return ConflictingSets;
